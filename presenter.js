...
if (isPresenter) {
  ...
  document.getElementById("participantApp")?.classList.add("hidden");
  document.getElementById("presenterApp")?.classList.remove("hidden");
  ...
  function renderRoster(list){
    ...
    rosterEl.innerHTML = list.map(p=>`<span class="player-roster-chip ${p.finished?'finished':''}"><b>${p.initials||'--'}</b><small>${p.score||0}/${total}</small>${p.finished?'<em>🏁</em>':''}</span>`).join('');
  }
  ...
  function laneHTML(p){
    ...
    return `<div class="lane-card ${p.rank===1?'leader':''}">
      ...
      <div class="lane-center">
        ...
        <div class="lane-track"><div class="lane-runner" style="left:calc(8px + (100% - 108px) * ${pct/100})"><img class="runner-img" src="runner.png" alt="Corredor" aria-hidden="true"></div></div>
      </div>
  ...
  document.getElementById('openParticipantBtn')?.addEventListener('click', ()=>{
  ...
  if (!settings.enabled) {
    ...
    document.getElementById('resetGameBtn')?.addEventListener('click', async ()=>{
      const ok = window.confirm('Deseja iniciar uma nova partida? Isso limpará todos os jogadores e resultados atuais.');
      if(!ok) return;
